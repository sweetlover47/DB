drop table flyway_schema_history cascade ;

drop table rest_tourist cascade ;

drop table cargo_tourist cascade ;

drop table trip cascade ;

drop table agency cascade ;

drop table cargo cascade ;

drop table child cascade ;

drop table tourist cascade ;

drop table excursion cascade ;

drop table flight cascade ;

drop table passengers cascade ;

drop table airplane cascade ;

drop table room cascade ;

drop table hotel cascade ;

drop table statement cascade ;

drop table trans cascade ;

drop table warehouse cascade ;

drop sequence tourist_id_seq;

drop sequence trip_id_seq;

drop sequence agency_id_seq;

drop sequence airplane_id_seq;

drop sequence cargo_id_seq;

drop sequence child_id_seq;

drop sequence excursion_id_seq;

drop sequence flight_id_seq;

drop sequence hotel_id_seq;

drop sequence passengers_id_seq;

drop sequence room_id_seq;

drop sequence statement_id_seq;

drop sequence transaction_id_seq;

drop sequence warehouse_id_seq;